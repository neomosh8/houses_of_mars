...
const FIRST_PROMPTS = {
  WatOx: 'You are our water-oxygen extraction expert. How can we increase production? Give one concise idea. If you need more info, ask one sentence.'
}
...
class WorkforceChatManager {
  ...
  async _step(key) {
    ...
    for (const worker of chat.workers) {
      ...
      // only directors get the JSON-output instruction
      if (worker.director) {
        instructions += ' When you are ready to propose, reply with JSON exactly like { "dialogue": "...", "is_proposal": true, "proposal": { ... } }. Otherwise reply with { "dialogue": "...", "is_proposal": false, "proposal": null }.'
      }

      const prompt = !worker.initialized && history
        ? `${history}\n${chat.firstPrompt}`
        : history || chat.firstPrompt

      let result
      try {
        result = await this._query(instructions, prompt)
      } catch (err) {
      ...
      if (result.dialogue) {
        if (worker.director) {
          // store an object for the client to parse
          const payload = {
            dialogue: result.dialogue,
            is_proposal: result.is_proposal,
            proposal: result.proposal || null
          }
          chat.messages.push({ worker: worker.name, text: payload })
        } else {
          // plain text for non-directors
          chat.messages.push({ worker: worker.name, text: result.dialogue })
        }
        worker.initialized = true
      }

      // if director made a proposal, record it
  ...
  resolveProposal(instId, index, status) {
    ...
    const chat = this.chats[key]
    if (!chat) return
    if (chat.pendingProposal?.instId === instId && chat.pendingProposal.index === index) {
